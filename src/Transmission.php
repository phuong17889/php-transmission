...
class Transmission {
	...
	public function all() {
		...
		$response = $this->getClient()->call('torrent-get', array('fields' => array_keys(Torrent::getMapping())));
		$torrents = array_map(function($data) use ($mapper, $client) {
			return $mapper->map(new Torrent($client), $data);
	...
	public function get($id) {
		...
		$torrent  = array_reduce($this->validator->validate('torrent-get', $response), function($torrent, $data) use ($mapper, $client) {
	...
	public function getSession() {
		$response = $this->getClient()->call('session-get', array());
		return $this->getMapper()->map(new Session($this->getClient()), $this->getValidator()->validate('session-get', $response));
	}

	public function getSessionStats() {
		$response = $this->getClient()->call('session-stats', array());
		return $this->getMapper()->map(new SessionStats(), $this->getValidator()->validate('session-stats', $response));
	}
	...
	public function getFreeSpace($path = null) {
		...
		$response = $this->getClient()->call('free-space', array('path' => $path));
		return $this->getMapper()->map(new FreeSpace(), $this->getValidator()->validate('free-space', $response));
	}
	...
	public function start(Torrent $torrent, $now = false) {
		$this->getClient()->call($now ? 'torrent-start-now' : 'torrent-start', array('ids' => array($torrent->getId())));
	}
	...
	public function stop(Torrent $torrent) {
		$this->getClient()->call('torrent-stop', array('ids' => array($torrent->getId())));
	}
	...
	public function verify(Torrent $torrent) {
		$this->getClient()->call('torrent-verify', array('ids' => array($torrent->getId())));
	}
	...
	/**
	 * Set the client used to connect to \transmission
	 *
	 * @param Client $client
	 */
	...
	/**
	 * Set the mapper used to map responses from Transmission to modelss
	 *
	...
	/**
	 * Get the mapper used to map responses from Transmission to modelss
	 *
